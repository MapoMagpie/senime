...
| senime-lsp | 编辑器语言服务，以补全方式输入中文 | `senils` |
| senime-fcitx5 | Fcitx5 输入法插件 | `senime-fcitx5/build/libsenime.so` |
...
```bash
nix develop          # 或 nix develop .#rust
```
...
senime-fcitx5 的 C++ 代码使用 clangd 提供补全和跳转。首次使用或系统更新后，在 nix 开发环境中生成 `compile_commands.json`：

```bash
cd senime-fcitx5 && cmake -B build . -DCMAKE_EXPORT_COMPILE_COMMANDS=ON
```

clangd 会自动读取 `build/compile_commands.json`，所有头文件路径（nix store、fcitx5、项目的 `include/`）均由 CMake 动态生成，无需手动维护。
