...
//! 1. 先用 `InputAnalyzer::default()` 占位并从锁中取出旧引擎，drop 释放内存
//! 2. 再加载新引擎并写入锁中
//!
//! 这样确保旧引擎的内存在加载新引擎之前已经完全释放，内存峰值
//! 不会超过 1x + 占位符的大小（Default InputAnalyzer 很小）。仅响应被
//! 监听文件的实质性变更（创建/修改/删除），忽略 `Access`（打开/关闭/读取）
//! 事件，避免重建时读取码表触发 `IN_OPEN` 形成无限循环。
//!
